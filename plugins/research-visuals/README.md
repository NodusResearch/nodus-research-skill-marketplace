...
| [Historical Maps](skills/historical-maps/SKILL.md) | `research-visuals:cartography/render-historical-map` | Dated historical geometry, places and routes; sources that cover the request and link to evidence are reported as dated, and everything else is drawn as a labelled approximate reconstruction |
...
Historical Maps draws **the dated map it was asked for**, from whatever the request can supply: GeoJSON layers, coordinates for named places, routes of two or more positions, and a provider query used as a **reference frame**. A source that declares a period covering the request **and** carries an evidence link makes the result a dated map, reported with that period; every other supplied layer or overlay renders as an approximate reconstruction, and the returned map carries a warning notice and a paragraph saying so, so the label travels into any document that embeds it. A reference layer is drawn and attributed, never makes the map dated, and is stated in the map as today's boundaries: that is how a request for provinces, regions or states is answered when only the current division is published. There is no historical border provider. An opaque dataset handle is refused, as is a declared source period that does not cover the requested interval — a claim the source cannot support is the one thing that would make the label untrue. Dates use ISO format, including expanded signed years for BCE (`-000500-01-01`). Every source still needs label, attribution and licence, and these consistency checks cannot establish historical accuracy: the researcher remains responsible for the evidence, including when the map is an approximation.
...
Run deterministic tests with `node --test plugins/research-visuals/test/*.test.mjs`. They cover relevant selection, all rejected, text-only fallback, second-batch success, checked sources, licensing, changed image bytes, forged receipts, cancellation and the historical rules (dated, reconstructed, refused substitution). Nodus supplies the native geometry/projection, model-provider, sandbox and chat-surface tests. Test image/geometry fixtures are synthetic; no paid calls or personal data are required.
